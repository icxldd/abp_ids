...
namespace Icxl.Abp.Ids.IdentityServer
{
    public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        ...
        private async Task CreateApiResourcesAsync()
        {
            ...
            await CreateApiResourceAsync("Ids", commonApiUserClaims);
        }
        ...
        private async Task CreateClientsAsync()
        {
            ...
            var consoleAndAngularClientId = configurationSection["Ids_App:ClientId"];
            if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
            {
                var webClientRootUrl = configurationSection["Ids_App:RootUrl"]?.TrimEnd('/');

                await CreateClientAsync(
                    name: consoleAndAngularClientId,
                    scopes: commonScopes,
                    grantTypes: new[] {"password", "client_credentials", "authorization_code"},
                    secret: (configurationSection["Ids_App:ClientSecret"] ?? "1q2w3e*").Sha256(),
            ...
            // Swagger Client
            var swaggerClientId = configurationSection["Ids_Swagger:ClientId"];
            if (!swaggerClientId.IsNullOrWhiteSpace())
            {
                var swaggerRootUrl = configurationSection["Ids_Swagger:RootUrl"].TrimEnd('/');

                await CreateClientAsync(
                    name: swaggerClientId,
                    scopes: commonScopes,
                    grantTypes: new[] {"authorization_code"},
                    secret: configurationSection["Ids_Swagger:ClientSecret"]?.Sha256(),
                    requireClientSecret: false,
